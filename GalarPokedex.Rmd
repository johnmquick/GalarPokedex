...
* `fCheckVariants`: Given the page content for a Pokémon, checks whether variants exist and returns a corresponding logical value
* `fGetData`: Given a file, directs data extraction for variants and non-variants accordingly, then returns one or more complete rows representing a Pokémon and its variants, if any
* `fGetDataCommon`: Given the page content for a Pokémon, extracts, cleans, and returns all data that is the same, regardless of whether variants exist
* `fGetDataNormal`: Given the page content for a Pokémon, extracts, cleans, and returns all data that applies to Pokémon that do not have variants
* `fGetDataVariant`: Given the page content for a Pokémon, extracts, cleans, and returns all data that applies to Pokémon that have variants 
